...
class SvddSGD(object):
    ...
    def fits(self, X, max_iter=20000, prec=1e-6, rate=0.01):
        if X.shape[1] < 1:
            ...
            return -1, -1
        c, radius2, pobj, iter = fit_extern(self.c, X, self.nu, max_iter, prec, rate)
        self.c = c
        self.radius2 = radius2
        self.pobj = pobj
        print 'SDG_iter={0}: object={1} R={2}'.format(iter + 1, self.pobj, self.radius2)
...
def fit_extern(last_c, X, nu, max_iter, prec, rate):
    ...
    while not is_converged and iter < max_iter:
        ...
        T -= np.sign(dT) * max_change  # changes
