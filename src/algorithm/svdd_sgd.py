...
class SvddSGD(object):
    ...
    def fits(self, X, max_iter=20000, prec=1e-6, rate=0.01):
        if X.shape[1] < 1:
            ...
            return -1, -1
        self.c, self.radius2, self.pobj, iter = optimal(X, self.nu, max_iter, prec, rate)
        print 'SDG_iter={0}: object={1} R={1}'.format(iter + 1, self.pobj, self.radius2)
...
def optimal(X, nu, max_iter, prec, rate):
    ...
    while not is_converged and iter < max_iter:
        ...
        # calculate the distances of the center to each data point
        dist = np.sum(c * c) - dot_2cX + sum_XX
        inds_size = 0
        for s in range(samples):
            if dist[s] - T >= 1e-12:
